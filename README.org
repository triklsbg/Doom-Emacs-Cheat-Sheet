...
#+OPTIONS: toc:1  ; 2 level headlines, for exported files (= toc-usecase 1)

# (toc-usecase 2) a toc inside the org file managed by toc-org package/feature
#+TOC: headlines 1

* Contents    :TOC:
- [[#a-few-tips-for-emacs-newbies][A few tips for Emacs newbies]]
- [[#command-abbreviations][Command Abbreviations]]
- [[#modal-editing][Modal Editing]]
  - [[#visual-mode][Visual mode]]
  - [[#operator-pending-state][Operator-pending state]]
- [[#general][General]]
- [[#projects-with-projectile][Projects with Projectile]]
- [[#dired-the-emacs-builtin-file-explorer][*Dired* the emacs builtin file explorer]]
...
- [[#org-mode][Org Mode]]
  ...
  - [[#defining-custom-link-types][Defining custom Link Types]]
  - [[#linking-to-words--bookmarks][Linking to words & Bookmarks]]
  - [[#code-snippets--babel][Code Snippets & Babel]]
  - [[#task-management][Task Management]]
  - [[#lists][Lists]]
  - [[#items-checkboxessubtasks][Items (Checkboxes/Subtasks)]]
  - [[#misc][Misc]]
- [[#magit][Magit]]
...
- [[#file-tree][File Tree]]
- [[#capturing][Capturing]]
  - [[#output-of-shell-command][Output of shell command]]
- [[#org-roam][Org Roam]]
- [[#code-folding][Code Folding]]
- [[#pdf-view-mode][pdf-view-mode]]
  - [[#zoom-and-scroll][Zoom and scroll]]
- [[#latex][Latex]]
- [[#bi-directinal-texts-rtl-languages][Bi-directinal texts (RTL languages)]]
- [[#sources][Sources]]
- [[#what-to-learn-03][What to learn]]
...
- =SPC= - "Space" key
* Modal Editing
- =ESC= brings you from insert Mode to Normal Mode. Cursor shape is a letter wide block. The indicator in the status line is =N=. The keys you enter are interpreted as commands.
- =i= changes to Insert Mode from Normal Mode. The cursor changes to a small vertical line. The status line indicater is =I=. The keys you enter are recorded as new text.
** Visual mode
- =V= selects complete lines. Background turns to grey
- =v= allows you to select rectangular blocks in a text, grey background
** Operator-pending state
Special state entered after launching an operator, but before specifying the corresponding motion or text object. Cursor shape changes to a little square. Indicator =O=. Example: Start a deletion with =d= evil waites for text object.
The selected region can be copied, deleted or replaced with special commands.
* General
...
A project in the sense of /projectile/ is a directory containing a =.git= repository.
- =SPC p p= to open an existing project
- =SPC SPC= to open a file in a project
- =SPC o p= to open the project side bar (file explorer)
...
- =M-x projectile-discover-projects-in-search-path= to find projects in
  ...
  using Projectile
* *Dired* the emacs builtin file explorer
...
- Buffers are a special concept in emacs they can be terminals, files,
  directories. So called system buffers have * (earmuffs) around their name.
- =SPC b b= to open another buffer
  - workspace buffer
- =SPC b B= you can see all (ordinary-, system-, workspace-) buffers
- =SPC ,= to switch buffers in the current? workspace
# - =SPC SHIFT ,= to switch to all buffers
- =SPC b X= Create or revisit a persistant =*doom:scratch*= buffer
...
- =SPC TAB .= Switch between known workspaces by name
- =SPC TAB [= Previous workspace, the tabbar highlights the current workspace
...
- You can't assign a tag or a priority to items, only to headings.
*** Turning normal lines into items and vice verca
- =SPC m i= to turn lines into items and vice verca.
- =SPC m h= to turn lines into org headings and vice verca.
- =SPC m s n= to focus on a single heading by hiding all others and =SPC m s N= to undo.
** Misc
...
- Forge is installed in emacs doom via /(magit +forge)/ flag in init.el
- =@= for forge
...
* File Tree
Think about /(dired +dirvish)/ flag
...
- =!!<shell-command>= replaces the current line with the output of /shell-command/. 
* Org Roam
